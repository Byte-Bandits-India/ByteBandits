...
export interface StaggeredMenuItem {
  ...
  ariaLabel: string;
  link: string;
  type?: 'route' | 'section' | 'external'; // Added to handle different navigation types
}
...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  // Aliases/utilities to match requested API
  const isSidebarOpen = open;
  const toggleSidebar = () => toggleMenu();
  const scrollToContact = useCallback(() => {
    router.push('/contact');
  }, [router]);

  // Navigation handler similar to your Navbar component
  const handleNavigation = useCallback((item: StaggeredMenuItem) => {
    // Close menu first (inline close to avoid referencing toggleMenu before it's declared)
    if (openRef.current) {
    ...
    if (item.type === 'external' || link.startsWith('http')) {
      // External link
      window.open(link, '_blank');
    } else if (link.startsWith('#')) {
      // Section scroll (like your Services section)
  ...
  }, [router, onNavigation, onMenuClose]);

  // Enhanced toggle menu function (moved below dependent callbacks to avoid TDZ)

  // Add click handler for menu items
  const handleMenuItemClick = useCallback((e: React.MouseEvent, item: StaggeredMenuItem) => {
    e.preventDefault();
    handleNavigation(item);
  }, [handleNavigation]);

  // Rest of your existing GSAP animation functions remain the same
  useLayoutEffect(() => {
  ...
  // Define toggleMenu after dependent callbacks to avoid TDZ errors
  const toggleMenu = useCallback(() => {
    ...
    if (target) {
      onMenuOpen?.();
      // Ensure DOM (prelayers/panel) is mounted before building timeline
      if (typeof window !== 'undefined' && 'requestAnimationFrame' in window) {
        requestAnimationFrame(() => playOpen());
      } else {
        setTimeout(() => playOpen(), 0);
    ...
    <div className="sm-scope w-full max-w-[1767px] mx-auto h-full">
      ...
      >
        ...
        <aside
          ...
          ref={panelRef}
          className="staggered-menu-panel absolute top-0 right-0 h-full bg-white flex flex-col p-[6em_2em_2em_2em] overflow-y-auto z-50 backdrop-blur-[12px]"
          style={{ WebkitBackdropFilter: 'blur(12px)' }}
          aria-hidden={!open}
        >
          <div className="sm-panel-inner flex-1 flex flex-col gap-5">
            <ul
              className="sm-panel-list list-none p-0 flex flex-col gap-2 mt-10"
              role="list"
      ...
      <style>{`
        ...
        /* Hide panel and prelayers offscreen by default to prevent flash before JS sets transforms */
